...
#define _CRT_SECURE_NO_DEPRECATE // NEED IT TO COMPILE NORMALY, #SHTUPID STUDIO

#include <iostream>
#include <cstring>
#include <stdio.h>

//#include <math.h>	// Included this Library for Pow(base, power # raised) == base^power # raised
// commented out library since it won't be used
...
strcpy(t[i][0], null_st); // strcpy(t[i][0], null_st);
strcpy(t[i][1], null_st);	// strcpy(t[i][1], null_st);
}
...
strcpy(temp_s, s);	// strcpy(temp_s, s);
...
strcpy(t[pos][index], temp_s);	//strcpy(t[pos][index], temp_s);
placed = true;
return placed;
}
else {
...
strcpy(temp_s, temp);	// strcpy(temp_s, temp);
...
// HASH FUNCTION
	...
	for (int Aiko = 0; Aiko < len; Aiko++) {
		//val += (s[len - Aiko - 1] * pow(31, Aiko));
		val += s[len - Aiko - 1] * po;
		po *= 31;
		po = po % tablesize;
	}
