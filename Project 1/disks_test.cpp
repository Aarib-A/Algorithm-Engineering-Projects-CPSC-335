...
int main() {
  ...
  rubric.criterion("disk_state::is_alternating", 3,
     		   [&]() {
             TEST_TRUE("is_alternating() for n=1", alt_one.is_alternating());
             TEST_TRUE("is_alternating() for n=1", alt_three.is_alternating());
             TEST_FALSE("is_alternating() after swap", sorted_one.is_alternating());
             TEST_FALSE("is_alternating() after swaps", sorted_three.is_alternating());
           });
  ...
  rubric.criterion("left-to-right, n=4", 1,
     		   [&]() {
             auto output = sort_left_to_right(disk_state(4));
             TEST_TRUE("actually sorted", output.after().is_sorted());
             TEST_EQUAL("number of swaps must be 10", 10, output.swap_count());
           });

  rubric.criterion("left-to-right, n=3", 1,
     		   [&]() {
             auto output = sort_left_to_right(disk_state(3));
             TEST_TRUE("actually sorted", output.after().is_sorted());
  ...
  rubric.criterion("left-to-right, other values", 1,
             ...
             auto trial = [](unsigned n) {
               return sort_left_to_right(disk_state(n)).swap_count();
             };
